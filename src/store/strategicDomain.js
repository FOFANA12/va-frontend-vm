...
export const useStrategicDomainStore = defineStore('strategicDomain', () => {
  ...
  const getAll = async ({
    ...
    const { data } = await api.get(endpoints.strategicDomain.getAll, { params });
    strategicDomains.value = data.data;
    meta.value = data.meta;
  };

  const find = async (id, mode = 'edit') => {
    try {
      const response = await api.get(endpoints.strategicDomain.find(id, mode));
      Object.assign(form.value, response.data.strategic_domain);
      return response.data;
    } catch (error) {
      if (error?.response?.data) throw error.response.data;
      throw { message: 'Unable to fetch strategic domain.' };
    }
  };
  ...
  const create = async () => {
    ...
    try {
      const response = await api.post(endpoints.strategicDomain.create, form.value.data());
      return response.data;
    } catch (error) {
      form.value.errors.set(error.response?.data?.errors || {});
      if (error?.response?.data) throw error.response.data;
      throw { message: 'Failed to create strategic domain.' };
    } finally {
  ...
  const update = async (id) => {
    ...
    form.value.busy = true;
    try {
      const response = await api.put(endpoints.strategicDomain.update(id), form.value.data());
      resetForm();
      Object.assign(form.value, response.data.strategic_domain);
      ...
      form.value.errors.set(error.response?.data?.errors || {});
      if (error?.response?.data) throw error.response.data;
      throw { message: 'Failed to update strategic domain.' };
    } finally {
      form.value.busy = false;
    }
  };
  ...
  const destroy = async (ids) => {
    try {
      const response = await api.post(endpoints.strategicDomain.destroy, { ids });
      return response.data;
    } catch (error) {
      if (error?.response?.data) throw error.response.data;
      throw { message: 'Failed to delete strategic domain.' };
    }
  };
  ...
  const requirements = async () => {
    ...
    try {
      const { data } = await api.get(endpoints.strategicDomain.requirements);
