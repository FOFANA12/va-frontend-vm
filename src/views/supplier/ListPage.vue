<template>
  <DefaultLayout>
    ...
    >
      <!-- Breadcrumbs -->
      <Breadcrumbs :breadcrumbs="breadcrumbs" :pageTitle="t('supplier.list.breadTitle')" />

      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_SUPPLIER) && selectedRows.length > 0"
          :icon="Trash"
...
const breadcrumbs = [
  ...
  {
    title: 'supplier.list.breadActive',
    disabled: true,
    to: {
      name: 'supplier',
    },
...
const columns = getColumns({
  t,
  onView: (id) => router.push({ name: 'supplier-show', params: { id } }),
  onEdit: (id) => router.push({ name: 'supplier-edit', params: { id } }),
