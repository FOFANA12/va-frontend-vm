<template>
  <DefaultLayout>
    ...
    >
      <Breadcrumbs :breadcrumbs="route.meta.breadcrumbs" :pageTitle="t('structure.breadTitle')" />
      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_STRUCTURE) && selectedRows.length > 0"
          :icon="Trash"
        ...
        <Button
          v-if="hasPermission(PERMISSIONS.CREATE_STRUCTURE)"
          :icon="Plus"
          variant="primary"
          customClass="sm:px-4"
