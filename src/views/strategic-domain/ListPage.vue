<template>
  <DefaultLayout>
    ...
    >
      <!-- Breadcrumbs -->
      <Breadcrumbs :breadcrumbs="breadcrumbs" :pageTitle="t('strategicDomain.list.breadTitle')" />

      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_STRATEGIC_DOMAIN) && selectedRows.length > 0"
          :icon="Trash"
...
const breadcrumbs = [
  ...
  {
    title: 'sidebar.strategicDomains',
    disabled: true,
    to: null,
  },
  {
    title: 'strategicDomain.list.breadActive',
    disabled: true,
    to: {
...
const columns = getColumns({
  t,
  onView: (id) => router.push({ name: 'strategicDomain-show', params: { id } }),
  onEdit: (id) => router.push({ name: 'strategicDomain-edit', params: { id } }),
