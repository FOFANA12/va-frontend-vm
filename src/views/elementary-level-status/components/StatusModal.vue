...
const form = elementaryLevelStatusStore.form;

const closeModal = () => {
  isModalOpen.value = false;
  elementaryLevelStatusStore.resetForm();
};

const onSubmit = async () => {
  try {
    const result = await elementaryLevelStatusStore.create(currentElementaryLevelId.value);
    
    elementaryLevelStore.form.status = result.status.status;
    elementaryLevelStore.form.status_changed_at = result.status.status_date;
    elementaryLevelStore.form.status_changed_by = result.status.author;
...
const loadRequirements = async (programId) => {
  try {
    const result = await elementaryLevelStatusStore.requirements(programId);
