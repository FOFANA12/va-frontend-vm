  ...
  >
    ...
    <div class="w-full bg-white mx-auto rounded-lg mb-4 overflow-hidden">
      <table class="min-w-full table-auto border-collapse">
        <thead class="bg-white text-gray-700">
          <tr>
            <th class="w-2/5 px-4 py-1.5 border-b border-gray-300 text-left">
              {{ t('elementaryLevelState.table.date') }}
            </th>
            <th class="w-2/5 px-4 py-1.5 border-b border-gray-300 text-left">
        ...
        <tbody>
          <tr v-if="!elementaryLevelStateStore.states || elementaryLevelStateStore.states.length === 0">
            <td colspan="4" class="bg-white text-center py-4 text-gray-500 border border-gray-100">
              {{ t('elementaryLevelState.table.noData') }}
            </td>
          </tr>
          <tr
            v-for="item in elementaryLevelStateStore.states"
            :key="item.uuid"
          ...
          >
            ...
            <td class="px-4 py-2 border-t border-gray-200 text-center">
              <button
                v-if="elementaryLevelStateStore.states.length > 1"
                type="button"
...
const {
  ...
  await elementaryLevelStateStore.getAll(route.params.id);
});
...
const deleteRows = async (ids) => {
  ...
  if (confirm.isConfirmed) {
    try {
      const result = await elementaryLevelStateStore.destroy(ids, route.params.id);
      showSimpleAlerte({ icon: 'success', text: result.message });

      elementaryLevelStore.form.state = result.data?.state ?? null;
      elementaryLevelStore.form.state_changed_at = result.data?.state_date ?? null;
      elementaryLevelStore.form.state_changed_by = result.data?.author ?? null;
