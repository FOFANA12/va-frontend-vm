<template>
  <DefaultLayout>
    ...
    >
      <!-- Breadcrumbs -->
      <Breadcrumbs :breadcrumbs="breadcrumbs" :pageTitle="t('employee.list.breadTitle')" />

      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_EMPLOYEE) && selectedRows.length > 0"
          :icon="Trash"
...
const breadcrumbs = [
  ...
  {
    title: 'employee.list.breadActive',
    disabled: true,
    to: {
      name: 'employee',
    },
...
const columns = getColumns({
  t,
  onView: (id) => router.push({ name: 'employee-show', params: { id } }),
  onEdit: (id) => router.push({ name: 'employee-edit', params: { id } }),
