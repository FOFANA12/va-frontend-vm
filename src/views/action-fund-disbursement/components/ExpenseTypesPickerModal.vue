<template>
  <CustomModal size="lg" :isModalOpen="isModalOpen" @update:isModalOpen="closeModal">
    <template #title>
      <h2 class="text-md font-semibold">{{ t('actionFundDisbursement.expenseTypes.modalTitle') }}</h2>
    </template>
            ...
            <thead class="bg-gray-50 text-gray-700">
              <tr>
                ...
                <th class="text-left p-2 border">
                  {{ t('actionFundDisbursement.expenseTypes.name') }}
                </th>
              </tr>
            </thead>
            <tbody>
              <tr v-if="paginatedExpenseTypes.length === 0">
                <td colspan="3" class="text-center text-gray-500 py-4 border">
                  {{ t('common.noData') }}
                </td>
              </tr>
              <tr
                v-for="expenseType in paginatedExpenseTypes"
              ...
              >
                <td class="p-2 border text-center">
                  <input
                    type="checkbox"
                    v-model="selectedExpenseTypes"
                    :value="expenseType"
                    class="checkbox-primary h-5 w-5"
                  />
                </td>
                <td class="p-2 border">{{ expenseType.name }}</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <template #buttons>
      ...
      >
        {{ t('actionFundDisbursement.expenseTypes.btnValidate') }}
      </button>
    </template>
  </CustomModal>
</template>
...
const selectedExpenseTypes = ref([]);
...
const filteredExpenseTypes = computed(() => {
  return store.expenseTypes.filter((b) =>
    b.name.toLowerCase().includes(localSearch.value.toLowerCase())
  );
});

// Pagination
const paginatedExpenseTypes = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage.value;
  return filteredExpenseTypes.value.slice(start, start + itemsPerPage.value);
});

const totalPages = computed(() =>
  Math.ceil(filteredExpenseTypes.value.length / itemsPerPage.value)
);
...
const allChecked = computed(() => {
  const current = paginatedExpenseTypes.value;
  return (
    current.length > 0 &&
    current.every((b) => selectedExpenseTypes.value.some((s) => s.uuid === b.uuid))
  );
});

const toggleSelectAll = () => {
  const current = paginatedExpenseTypes.value;
  const selectedIds = selectedExpenseTypes.value.map((b) => b.uuid);

  if (allChecked.value) {
    selectedExpenseTypes.value = selectedExpenseTypes.value.filter(
      (b) => !current.some((c) => c.uuid === b.uuid)
    );
  } else {
    const newItems = current.filter((b) => !selectedIds.includes(b.uuid));
    selectedExpenseTypes.value.push(...newItems);
  }
};

const confirmSelection = () => {
  emit('onSelect', selectedExpenseTypes.value);
  closeModal();
};
