...
<script setup>
import { useIndicatorStatusStore,  useIndicatorStore} from '@/store';
const indicatorStatusStore = useIndicatorStatusStore();
...
const form = indicatorStatusStore.form;

const closeModal = () => {
  isModalOpen.value = false;
  indicatorStatusStore.resetForm();
};

const onSubmit = async () => {
  try {
    const result = await indicatorStatusStore.create(currentIndicatorId.value);
    
    indicatorStore.form.status = result.status.status;
    indicatorStore.form.status_changed_at = result.status.status_date;
    indicatorStore.form.status_changed_by = result.status.author;
...
const loadRequirements = async (indicatorId) => {
  try {
    const result = await indicatorStatusStore.requirements(indicatorId);
...
const openStatusModal = async (indicatorId) => {
  currentIndicatorId.value = indicatorId;
  indicatorStore.resetForm();

  try {
    await loadRequirements(indicatorId);
    isModalOpen.value = true;
  } catch (err) {
