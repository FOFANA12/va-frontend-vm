  ...
  >
    ...
    <div class="w-full bg-white mx-auto rounded-lg mb-4 overflow-hidden">
      <table class="min-w-full table-auto border-collapse">
        ...
        <tbody>
          <tr v-if="!indicatorStatusStore.statuses || indicatorStatusStore.statuses.length === 0">
            <td colspan="4" class="bg-white text-center py-4 text-gray-500 border border-gray-100">
              {{ t('indicatorStatus.table.noData') }}
            </td>
          </tr>
          <tr
            v-for="item in indicatorStatusStore.statuses"
            :key="item.uuid"
...
const {
  ...
  await indicatorStatusStore.getAll(route.params.id);
});
...
const deleteRows = async (ids) => {
  ...
  if (confirm.isConfirmed) {
    try {
      const result = await indicatorStatusStore.destroy(ids, route.params.id);
      showSimpleAlerte({ icon: 'success', text: result.message });

      indicatorStore.form.status = result.status?.status ?? null;
      indicatorStore.form.status_changed_at = result.status?.status_date ?? null;
      indicatorStore.form.status_changed_by = result.status?.author ?? null;
