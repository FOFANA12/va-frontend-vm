<template>
  <DefaultLayout>
    ...
    >
      <!-- Breadcrumbs -->
      <Breadcrumbs :breadcrumbs="breadcrumbs" :pageTitle="t('strategicLever.list.breadTitle')" />

      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_STRATEGIC_LEVER) && selectedRows.length > 0"
          :icon="Trash"
        ...
        <LinkButton
          v-if="hasPermission(PERMISSIONS.CREATE_STRATEGIC_LEVER)"
...
const breadcrumbs = [
  ...
  {
    title: 'strategicLever.list.breadActive',
    disabled: true,
    to: {
...
const columns = getColumns({
  t,
  onView: (id) => router.push({ name: 'strategic-levers-show', params: { id } }),
  onEdit: (id) => router.push({ name: 'strategic-levers-edit', params: { id } }),
