...
const form = capabilityDomainStateStore.form;

const closeModal = () => {
  isModalOpen.value = false;
  capabilityDomainStateStore.resetForm();
};

const onSubmit = async () => {
  try {
    const result = await capabilityDomainStateStore.create(currentCapabilityDomainId.value);
    capabilityDomainStore.form.state = result.state.state;
    capabilityDomainStore.form.state_changed_at = result.state.state_date;
    capabilityDomainStore.form.state_changed_by = result.state.author;
...
const loadRequirements = async (programId) => {
  try {
    const result = await capabilityDomainStateStore.requirements(programId);
