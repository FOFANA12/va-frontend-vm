<template>
  ...
  >
    ...
    <div class="flex justify-end mt-4 mb-6 gap-4">
      ...
      <Button 
      v-if="hasPermission(PERMISSIONS.ACT_MANAGE_STATUS)"
      :icon="Plus" variant="primary" customClass="sm:px-4" @click="openStatusModal">
        {{ t('actionStatus.btnAdd') }}
      </Button>
    </div>
    ...
    <div class="w-full bg-white mx-auto rounded-lg mb-4 overflow-hidden">
      <table class="min-w-full table-auto border-collapse">
        ...
        <tbody>
          <tr v-if="!actionStatusStore.statuses || actionStatusStore.statuses.length === 0">
            <td colspan="4" class="bg-white text-center py-4 text-gray-500 border border-gray-100">
              {{ t('actionStatus.table.noData') }}
            </td>
          </tr>
          <tr
            v-for="item in actionStatusStore.statuses"
            :key="item.uuid"
          ...
          >
            ...
            <td class="px-4 py-2 border-t border-gray-200 text-center">
              <button
                v-if="hasPermission(PERMISSIONS.ACT_MANAGE_STATUS) && actionStatusStore.statuses.length > 1"
                type="button"
...
import { useActionStatusStore, useActionStore } from '@/store';
...
const {
  ...
  await actionStatusStore.getAll(route.params.id);
});
...
const deleteRows = async (ids) => {
  ...
  if (confirm.isConfirmed) {
    try {
      const result = await actionStatusStore.destroy(ids, route.params.id);
      showSimpleAlerte({ icon: 'success', text: result.message });

      actionStore.form.status = result.status?.status ?? null;
      actionStore.form.status_changed_at = result.status?.status_date ?? null;
      actionStore.form.status_changed_by = result.status?.author ?? null;
