...
const form = actionStatusStore.form;

const closeModal = () => {
  isModalOpen.value = false;
  actionStatusStore.resetForm();
};

const onSubmit = async () => {
  try {
    const result = await actionStatusStore.create(currentActionId.value);
    
    actionStore.form.status = result.status.status;
    actionStore.form.status_changed_at = result.status.status_date;
    actionStore.form.status_changed_by = result.status.author;
...
const loadRequirements = async (actionId) => {
  try {
    const result = await actionStatusStore.requirements(actionId);
...
const openStatusModal = async (actionId) => {
  currentActionId.value = actionId;
  actionStatusStore.resetForm();

  try {
    await loadRequirements(actionId);
    isModalOpen.value = true;
  } catch (err) {
