  ...
  >
    ...
    <div class="w-full bg-white mx-auto rounded-lg mb-4 overflow-hidden">
      <table class="min-w-full table-auto border-collapse">
        ...
        <tbody>
          <tr v-if="!projectStatusStore.statuses || projectStatusStore.statuses.length === 0">
            <td colspan="4" class="bg-white text-center py-4 text-gray-500 border border-gray-100">
              {{ t('projectStatus.table.noData') }}
            </td>
          </tr>
          <tr
            v-for="item in projectStatusStore.statuses"
            :key="item.uuid"
          ...
          >
            ...
            <td class="px-4 py-2 border-t border-gray-200 text-center">
              <button
                v-if="projectStatusStore.statuses.length > 1"
                type="button"
...
const {
  ...
  await projectStatusStore.getAll(route.params.id);
});
...
const deleteRows = async (ids) => {
  ...
  if (confirm.isConfirmed) {
    try {
      const result = await projectStatusStore.destroy(ids, route.params.id);
      showSimpleAlerte({ icon: 'success', text: result.message });

      projectStore.form.status = result.status?.status ?? null;
      projectStore.form.status_changed_at = result.status?.status_date ?? null;
      projectStore.form.status_changed_by = result.status?.author ?? null;
