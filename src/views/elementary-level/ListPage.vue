<template>
  <DefaultLayout>
    ...
    >
      <!-- Breadcrumbs -->
      <Breadcrumbs :breadcrumbs="breadcrumbs" :pageTitle="t('elementaryLevel.list.breadTitle')" />

      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_ELEMENTARY_LEVEL) && selectedRows.length > 0"
          :icon="Trash"
        ...
        <LinkButton
          v-if="hasPermission(PERMISSIONS.CREATE_ELEMENTARY_LEVEL)"
          :to="{ name: 'elementaryLevel-create' }"
          :icon="Plus"
          variant="primary"
          customClass="sm:px-4"
        >
          {{ t('elementaryLevel.btnAdd') }}
        </LinkButton>
      </div>
...
const breadcrumbs = [
  ...
  {
    title: 'sidebar.elementaryLevels',
    disabled: true,
    to: null,
  },
  {
    title: 'elementaryLevel.list.breadActive',
    disabled: true,
    to: {
...
const columns = getColumns({
  t,
  onView: (id) => router.push({ name: 'elementaryLevel-show', params: { id } }),
  onEdit: (id) => router.push({ name: 'elementaryLevel-edit', params: { id } }),
