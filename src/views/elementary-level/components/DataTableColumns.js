...
export function getColumns({ t, onEdit, onDelete, onView, formatCurrency, formatNumber }) {
  return [
    ...
    columnHelper.accessor('reference', {
      header: () => t('elementaryLevel.table.reference'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('name', {
      header: () => t('elementaryLevel.table.name'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('start_date', {
      header: () => t('elementaryLevel.table.startDate'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('end_date', {
      header: () => t('elementaryLevel.table.endDate'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('responsible', {
      header: () => t('elementaryLevel.table.responsible'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('budget', {
      header: () => t('elementaryLevel.table.budget'),
      cell: (info) => {
