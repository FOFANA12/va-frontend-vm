  ...
  >
    ...
    <div class="w-full bg-white mx-auto rounded-lg mb-4 overflow-hidden">
      <table class="min-w-full table-auto border-collapse">
        ...
        <tbody>
          <tr v-if="!activityStatusStore.statuses || activityStatusStore.statuses.length === 0">
            <td colspan="4" class="bg-white text-center py-4 text-gray-500 border border-gray-100">
              {{ t('activityStatus.table.noData') }}
            </td>
          </tr>
          <tr
            v-for="item in activityStatusStore.statuses"
            :key="item.uuid"
          ...
          >
            ...
            <td class="px-4 py-2 border-t border-gray-200 text-center">
              <button
                v-if="activityStatusStore.statuses.length > 1"
                type="button"
...
import { useActivityStatusStore, useActivityStore } from '@/store';
...
const activityStatusStore = useActivityStatusStore();
...
const {
  ...
  await activityStatusStore.getAll(route.params.id);
});
...
const deleteRows = async (ids) => {
  ...
  if (confirm.isConfirmed) {
    try {
      const result = await activityStatusStore.destroy(ids, route.params.id);
      showSimpleAlerte({ icon: 'success', text: result.message });

      activityStore.form.status = result.status?.status ?? null;
      activityStore.form.status_changed_at = result.status?.status_date ?? null;
      activityStore.form.status_changed_by = result.status?.author ?? null;
