...
const form = activityStatusStore.form;

const closeModal = () => {
  isModalOpen.value = false;
  activityStatusStore.resetForm();
};

const onSubmit = async () => {
  try {
    const result = await activityStatusStore.create(currentProgramId.value);
    
    activityStore.form.status = result.status.status;
    activityStore.form.status_changed_at = result.status.status_date;
    activityStore.form.status_changed_by = result.status.author;
...
const loadRequirements = async (programId) => {
  try {
    const result = await activityStatusStore.requirements(programId);
