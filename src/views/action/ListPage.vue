<template>
  <DefaultLayout>
    ...
    >
      <!-- Breadcrumbs -->
      <Breadcrumbs :breadcrumbs="breadcrumbs" :pageTitle="t('action.list.breadTitle')" />

      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_ACTION) && selectedRows.length > 0"
          :icon="Trash"
...
const breadcrumbs = [
  ...
  {
    title: 'sidebar.operationalManagement',
    disabled: true,
    to: null,
  },
  {
...
const columns = getColumns({
  t,
  onCopy: (id) => router.push({ name: 'action-create', params: { id } }),
  onView: (id) => router.push({ name: 'action-show', params: { id } }),
  onEdit: (id) => router.push({ name: 'action-edit', params: { id } }),
...
onMounted(async () => {
  ...
  const hasQueryFilters = route.query.status || route.query.state || route.query.nature;
