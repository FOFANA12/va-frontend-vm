<template>
  <DefaultLayout>
    ...
    >
      <!-- Breadcrumbs -->
      <Breadcrumbs :breadcrumbs="breadcrumbs" :pageTitle="t('actionFundReceipt.list.breadTitle')" />

      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_FUND_RECEIPT) && selectedRows.length > 0"
          :icon="Trash"
        ...
        <LinkButton
          v-if="hasPermission(PERMISSIONS.CREATE_FUND_RECEIPT)"
...
const breadcrumbs = [
  ...
  {
    title: 'sidebar.actionFundReceipts',
    disabled: true,
    to: null,
  },
  {
    title: 'actionFundReceipt.list.breadActive',
    disabled: true,
    to: {
      name: 'actionFundReceipt',
    },
  },
];
...
const columns = getColumns({
  ...
  onView: (id) => router.push({ name: 'action-fund-receipt-show', params: { id } }),
  onEdit: (id) => router.push({ name: 'action-fund-receipt-edit', params: { id } }),
