<template>
  ...
  >
    ...
    <DataTable
      :columns="columns"
      :data="store.strategicStakeholders"
      :meta="store.meta"
...
const {
  ...
  (args) => store.getAll({ ...args, strategicMapId: route.params.id }),
  { id: 'id', desc: true },
  store
);
...
const openCreateModal = () => modal.value.openCreateModal(route.params.id);
