<template>
  <DefaultLayout>
    ...
    >
      <!-- Breadcrumbs -->
      <Breadcrumbs :breadcrumbs="breadcrumbs" :pageTitle="t('strategicMap.list.breadTitle')" />

      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_STRATEGIC_MAP) && selectedRows.length > 0"
          :icon="Trash"
        ...
        <Button
          v-if="hasPermission(PERMISSIONS.CREATE_STRATEGIC_MAP)"
          :icon="Plus"
          variant="primary"
          customClass="sm:px-4"
          @click="openCreateModal"
        >
          {{ t('strategicMap.btnAdd') }}
        </Button>
      </div>
...
const breadcrumbs = [
  ...
  {
    title: 'sidebar.strategicMaps',
    disabled: true,
    to: null,
  },
  {
    title: 'strategicMap.list.breadActive',
    disabled: true,
    to: {
      name: 'strategicMap',
    },
...
const columns = getColumns({
  t,
  onDetails: (id) => router.push({ name: 'strategicMap-details', params: { id } }),
