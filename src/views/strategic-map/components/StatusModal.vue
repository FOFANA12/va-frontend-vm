...
const onSubmit = async () => {
  ...
  try {
    ...
    const result = await actionStore.updateStatus(form.id, payload);
...
const openStatusModal = async (actionId, currentStatus) => {
  resetForm();
  form.id = actionId;
  form.status = currentStatus.code;

  try {
    if (!actionStore.statuses.length) {
      await actionStore.getStatuses();
    }
