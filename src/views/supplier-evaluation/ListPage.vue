<template>
  ...
  >
    ...
    <DataTable
      :columns="columns"
      :data="store.supplierEvaluations"
      :meta="store.meta"
...
const createRoute = computed(() => ({
  name: route.name.replace(/evaluation.*/, 'evaluation-create'),
  params: {
    id: route.params.id,
  },
}));
...
const columns = getColumns({
  ...
  onView: (id) =>
    router.push({
      name: getRouteName('evaluation-show'),
      params: {
        id: route.params.id,
        supplierEvaluationId: id,
      },
    }),
  onEdit: (id) =>
    router.push({
      name: getRouteName('evaluation-edit'),
      params: {
        id: route.params.id,
        supplierEvaluationId: id,
      },
...
const deleteRows = async (ids) => {
  ...
  if (confirm.isConfirmed) {
    try {
      const result = await store.destroy(ids, route.params.id);
