<template>
  <DefaultLayout>
    ...
    >
      <!-- Breadcrumbs -->
      <Breadcrumbs :breadcrumbs="breadcrumbs" :pageTitle="t('project.list.breadTitle')" />

      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_PROJECT) && selectedRows.length > 0"
          :icon="Trash"
        ...
        <LinkButton
          v-if="hasPermission(PERMISSIONS.CREATE_PROJECT)"
...
const breadcrumbs = [
  ...
  {
    title: 'project.list.breadActive',
    disabled: true,
    to: {
      name: 'project',
    },
...
const columns = getColumns({
  t,
  onView: (id) => router.push({ name: 'project-show', params: { id } }),
  onEdit: (id) => router.push({ name: 'project-edit', params: { id } }),
