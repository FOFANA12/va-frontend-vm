<template>
  <DefaultLayout>
    ...
    >
      <!-- Breadcrumbs -->
      <Breadcrumbs :breadcrumbs="breadcrumbs" :pageTitle="t('strategicAxis.list.breadTitle')" />

      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_STRATEGIC_AXIS) && selectedRows.length > 0"
          :icon="Trash"
...
const breadcrumbs = [
  ...
  {
    title: 'strategicAxis.list.breadActive',
    disabled: true,
    to: {
      name: 'strategic-axes',
    },
...
const columns = getColumns({
  t,
  onView: (id) => router.push({ name: 'strategic-axes-show', params: { id } }),
  onEdit: (id) => router.push({ name: 'strategic-axes-edit', params: { id } }),
