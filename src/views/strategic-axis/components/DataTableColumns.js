...
export function getColumns({ t, onEdit, onDelete, onView }) {
  return [
    ...
    columnHelper.accessor('abbreviation', {
      header: () => t('strategicAxis.table.abbreviation'),
      cell: (info) => info.getValue(),
    }),
    columnHelper.accessor('name', {
      header: () => t('strategicAxis.table.name'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('strategic_map', {
      header: () => t('strategicAxis.table.strategicMap'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('structure', {
      header: () => t('strategicAxis.table.structure'),
      cell: (info) => info.getValue(),
    }),

     columnHelper.accessor('parent', {
      header: () => t('strategicAxis.table.parent'),
      cell: (info) => info.getValue(),
    }),
