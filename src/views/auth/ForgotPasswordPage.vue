        ...
        <div class="w-full bg-white px-6 py-7 rounded-xl border app-border-color">
          ...
          <form @submit.prevent="handleSubmit" @keydown="form.onKeydown($event)" class="space-y-4">
            ...
            <div class="mb-6">
              ...
              >
                {{ t('forgotPassword.form.email') }}<span class="required-star">*</span>
              </label>
              <div class="relative">
                ...
                <input
                  ...
                  class="w-full pl-10 pr-4 py-2.5 rounded-lg text-sm text-gray-800 bg-transparent border focus:outline-none"
                  :class="[
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (error?.response?.data?.message) {
      alert.value = {
        type: 'error',
        message: error?.response?.data?.message || t('forgotPassword.failed'),
      };
    }
  }
};
