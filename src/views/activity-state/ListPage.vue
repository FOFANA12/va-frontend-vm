  ...
  >
    ...
    <div class="w-full bg-white mx-auto rounded-lg mb-4 overflow-hidden">
      <table class="min-w-full table-auto border-collapse">
        ...
        <tbody>
          <tr v-if="!activityStateStore.states || activityStateStore.states.length === 0">
            <td colspan="4" class="bg-white text-center py-4 text-gray-500 border border-gray-100">
              {{ t('activityState.table.noData') }}
            </td>
          </tr>
          <tr
            v-for="item in activityStateStore.states"
            :key="item.uuid"
          ...
          >
            ...
            <td class="px-4 py-2 border-t border-gray-200 text-center">
              <button
                v-if="activityStateStore.states.length > 1"
                type="button"
...
const {
  ...
  await activityStateStore.getAll(route.params.id);
});
...
const deleteRows = async (ids) => {
  ...
  if (confirm.isConfirmed) {
    try {
      const result = await activityStateStore.destroy(ids, route.params.id);
      showSimpleAlerte({ icon: 'success', text: result.message });

      activityStore.form.state = result.state?.state ?? null;
      activityStore.form.state_changed_at = result.state?.state_date ?? null;
      activityStore.form.state_changed_by = result.state?.author ?? null;
