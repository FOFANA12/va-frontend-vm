...
const context = 'strategicObjective';
...
const onSubmit = async () => {
  ...
  try {
    ...
    const result = await store.updateStatus(form.id, payload);
...
const openStatusModal = async (objectiveId, currentStatus) => {
  resetForm();
  form.id = objectiveId;
  form.status = null;

  try {
    await store.getStatuses(objectiveId);
    isModalOpen.value = true;
  } catch (err) {
