<template>
  ...
  >
    ...
    <div class="flex justify-end mt-4 mb-4 gap-4">
      ...
      <Button
        v-if="hasPermission(PERMISSIONS.IND_MANAGE_DECISIONS) && selectedRows.length > 0"
        :icon="Trash"
      ...
      <LinkButton
        v-if="hasPermission(PERMISSIONS.IND_MANAGE_DECISIONS)"
        :to="createRoute"
        :icon="Plus"
        variant="primary"
        customClass="sm:px-4"
...
const {
  ...
  (args) => store.getAll({ ...args, decidableType: 'indicators', decidableId: route.params.id }),
  { id: 'id', desc: true },
  store
);
