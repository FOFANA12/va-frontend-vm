...
const { canManageDecision } = useIndicatorRules();
...
const props = defineProps({
  ...
  onEdit: Function,
  onDelete: Function,
  indicatorStatus: { type: [String, Object, null], required: false, default: null },
});

const allowed = computed(() => canManageDecision(props.indicatorStatus));
...
<template>
  <div class="flex justify-center gap-2">
    ...
    <button
      v-if="hasPermission(PERMISSIONS.IND_MANAGE_DECISIONS) && allowed"
      @click="onEdit?.(row.id)"
