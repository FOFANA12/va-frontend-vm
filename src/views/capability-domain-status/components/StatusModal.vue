...
const context = 'capabilityDomain';
const isModalOpen = ref(false);
const currentCapabilityDomainId = ref(null);

const form = capabilityDomainStatusStore.form;

const closeModal = () => {
  isModalOpen.value = false;
  capabilityDomainStatusStore.resetForm();
};

const onSubmit = async () => {
  try {
    const result = await capabilityDomainStatusStore.create(currentCapabilityDomainId.value);
    
    capabilityDomainStore.form.status = result.status.status;
    capabilityDomainStore.form.status_changed_at = result.status.status_date;
    capabilityDomainStore.form.status_changed_by = result.status.author;
...
const loadRequirements = async (programId) => {
  try {
    const result = await capabilityDomainStatusStore.requirements(programId);
...
const openStatusModal = async (programId) => {
  currentCapabilityDomainId.value = programId;
  capabilityDomainStatusStore.resetForm();
