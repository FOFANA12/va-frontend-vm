<template>
  <DefaultLayout>
    ...
    >
      <Breadcrumbs :breadcrumbs="route.meta.breadcrumbs" :pageTitle="t('actionPlan.breadTitle')" />
      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_ACTION_PLAN) && selectedRows.length > 0"
          :icon="Trash"
...
const columns = getColumns({
  t,
  onCopy: (id) => openEditModal(id, 'duplicate'),
  onExport: (url) => {
    window.open(url, '_blank');
  },
  onView: (id) => openViewModal(id),
  onEdit: (id) => openEditModal(id, 'edit'),
...
const openEditModal = (id, currentContext) => modal.value.openEditModal(id, currentContext);
