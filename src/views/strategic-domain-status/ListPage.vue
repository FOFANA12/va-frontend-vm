  ...
  >
    ...
    <div class="flex justify-end mt-4 mb-6 gap-4">
      ...
      <Button :icon="Plus" variant="primary" customClass="sm:px-4" @click="openStatusModal">
        {{ t('strategicDomainStatus.btnAdd') }}
      </Button>
    </div>
    ...
    <div class="w-full bg-white mx-auto rounded-lg mb-4 overflow-hidden">
      <table class="min-w-full table-auto border-collapse">
        <thead class="bg-white text-gray-700">
          <tr>
            <th class="w-2/5 px-4 py-1.5 border-b border-gray-300 text-left">
              {{ t('strategicDomainStatus.table.date') }}
            </th>
            <th class="w-2/5 px-4 py-1.5 border-b border-gray-300 text-left">
        ...
        <tbody>
          <tr v-if="!strategicDomainStatusStore.statuses || strategicDomainStatusStore.statuses.length === 0">
            <td colspan="4" class="bg-white text-center py-4 text-gray-500 border border-gray-100">
              {{ t('strategicDomainStatus.table.noData') }}
            </td>
          </tr>
          <tr
            v-for="item in strategicDomainStatusStore.statuses"
            :key="item.uuid"
          ...
          >
            ...
            <td class="px-4 py-2 border-t border-gray-200 text-center">
              <button
                v-if="strategicDomainStatusStore.statuses.length > 1"
                type="button"
...
const {
  ...
  await strategicDomainStatusStore.getAll(route.params.id);
});
...
const deleteRows = async (ids) => {
  ...
  if (confirm.isConfirmed) {
    try {
      const result = await strategicDomainStatusStore.destroy(ids, route.params.id);
      showSimpleAlerte({ icon: 'success', text: result.message });

      strategicDomainStore.form.status = result.status?.status ?? null;
      strategicDomainStore.form.status_changed_at = result.status?.status_date ?? null;
      strategicDomainStore.form.status_changed_by = result.status?.author ?? null;
