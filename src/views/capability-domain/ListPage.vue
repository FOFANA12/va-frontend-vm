<template>
  <DefaultLayout>
    ...
    >
      <!-- Breadcrumbs -->
      <Breadcrumbs :breadcrumbs="breadcrumbs" :pageTitle="t('capabilityDomain.list.breadTitle')" />

      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_CAPABILITY_DOMAIN) && selectedRows.length > 0"
          :icon="Trash"
...
const breadcrumbs = [
  ...
  {
    title: 'sidebar.capabilityDomains',
    disabled: true,
    to: null,
  },
  {
    title: 'capabilityDomain.list.breadActive',
    disabled: true,
    to: {
...
const columns = getColumns({
  t,
  onView: (id) => router.push({ name: 'capabilityDomain-show', params: { id } }),
  onEdit: (id) => router.push({ name: 'capabilityDomain-edit', params: { id } }),
