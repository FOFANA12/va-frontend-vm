...
export function getColumns({ t, onEdit, onDelete, onView }) {
  return [
    ...
    columnHelper.accessor('name', {
      header: () => t('settings.currency.table.name'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('code', {
      header: () => t('settings.currency.table.code'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('is_default', {
      header: () => t('settings.currency.table.isDefault.label'),
      cell: (info) =>
        h(StatusBadge, {
          variant: info.getValue() ? 'success' : 'error',
          defaultText: info.getValue()
            ? t('settings.currency.table.isDefault.yes')
            : t('settings.currency.table.isDefault.no'),
        }),
    }),
