  ...
  <DefaultLayout>
    ...
    >
      <Breadcrumbs
        ...
        :pageTitle="t('settings.indicatorCategory.breadTitle')"
      />
      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_INDICATOR_CATEGORY) && selectedRows.length > 0"
          :icon="Trash"
        ...
        <Button
          v-if="hasPermission(PERMISSIONS.CREATE_INDICATOR_CATEGORY)"
          :icon="Plus"
          variant="primary"
          customClass="sm:px-4"
          @click="openCreateModal"
        >
          {{ t('settings.indicatorCategory.btnAdd') }}
        </Button>
      </div>
      ...
      <DataTable
        :columns="columns"
        :data="store.indicatorCategories"
        :meta="store.meta"
