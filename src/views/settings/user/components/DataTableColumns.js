...
export function getColumns({ t, onEdit, onDelete, onView }) {
  return [
    ...
    columnHelper.accessor('name', {
      header: () => t('settings.user.table.name'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('email', {
      header: () => t('settings.user.table.email'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('phone', {
      header: () => t('settings.user.table.phone'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('lang', {
      header: () => t('settings.user.table.lang'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('role', {
      header: () => t('settings.user.table.role'),
      cell: (info) => info.getValue(),
    }),
