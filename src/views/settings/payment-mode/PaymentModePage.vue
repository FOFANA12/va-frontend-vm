  ...
  <DefaultLayout>
    ...
    >
      <Breadcrumbs
        ...
        :pageTitle="t('settings.paymentMode.breadTitle')"
      />
      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_PAYMENT_MODE) && selectedRows.length > 0"
          :icon="Trash"
        ...
        <Button  v-if="hasPermission(PERMISSIONS.CREATE_PAYMENT_MODE)" :icon="Plus" variant="primary" customClass="sm:px-4" @click="openCreateModal">
          {{ t('settings.paymentMode.btnAdd') }}
        </Button>
      </div>
...
import { useSettingsPaymentModeStore } from '@/store';
...
const store = useSettingsPaymentModeStore();
