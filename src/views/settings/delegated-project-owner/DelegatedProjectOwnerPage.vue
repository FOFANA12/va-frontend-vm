  ...
  <DefaultLayout>
    ...
    >
      <Breadcrumbs
        ...
        :pageTitle="t('settings.delegatedProjectOwner.breadTitle')"
      />
      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_DELEGATED_PROJECT_OWNER) && selectedRows.length > 0"
          :icon="Trash"
        ...
        <Button
          v-if="hasPermission(PERMISSIONS.CREATE_DELEGATED_PROJECT_OWNER)"
          :icon="Plus"
          variant="primary"
          customClass="sm:px-4"
          @click="openCreateModal"
        >
          {{ t('settings.delegatedProjectOwner.btnAdd') }}
        </Button>
      </div>
      ...
      <DataTable
        :columns="columns"
        :data="store.delegatedProjectOwners"
        :meta="store.meta"
