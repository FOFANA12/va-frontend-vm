  ...
  <DefaultLayout>
    ...
    >
      <Breadcrumbs
        ...
        :pageTitle="t('settings.department.breadTitle')"
      />
      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_DEPARTMENT) && selectedRows.length > 0"
          :icon="Trash"
        ...
        <Button
          v-if="hasPermission(PERMISSIONS.CREATE_DEPARTMENT)"
          :icon="Plus"
          variant="primary"
          customClass="sm:px-4"
          @click="openCreateModal"
        >
          {{ t('settings.department.btnAdd') }}
        </Button>
      </div>
