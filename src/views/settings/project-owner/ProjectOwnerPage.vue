  ...
  <DefaultLayout>
    ...
    >
      <Breadcrumbs
        ...
        :pageTitle="t('settings.projectOwner.breadTitle')"
      />
      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_PROJECT_OWNER) && selectedRows.length > 0"
          :icon="Trash"
        ...
        <Button
          v-if="hasPermission(PERMISSIONS.CREATE_PROJECT_OWNER)"
          :icon="Plus"
          variant="primary"
          customClass="sm:px-4"
          @click="openCreateModal"
        >
          {{ t('settings.projectOwner.btnAdd') }}
        </Button>
      </div>
