<template>
  <CustomModal size="md" :isModalOpen="isModalOpen" @update:isModalOpen="closeModal">
    <template #title>
      <h2 class="text-md font-semibold">{{ modalTitle }}</h2>
    </template>

    <template #body>
      <form @submit.prevent="onSubmit" @keyup.enter="onSubmit" @keydown="form.onKeydown($event)">
        <div class="grid grid-cols-12 gap-6">
          <!-- Region -->
          <div class="col-span-12">
            <InputReadonly
              v-if="isReadOnly"
              id="region"
              name="region"
              :label="t('settings.municipality.form.region')"
              :placeholder="t('settings.municipality.form.regionPlaceholder')"
              :modelValue="form.region"
              readonly
            />
            <SingleSelect
              v-else
              id="region"
              name="region"
              v-model="form.region"
              @update:modelValue="onChangeRegion"
              :options="requirements.regions"
              :label="t('settings.municipality.form.region')"
              :placeholder="t('settings.municipality.form.regionPlaceholder')"
              :error="form.errors.get('region')"
              :control-class="'px-3 py-2.5'"
              :dropdown-class="'max-h-60'"
              :option-class="'text-sm'"
              :empty-message="t('common.select.noResults')"
              :search-placeholder="t('common.select.searchPlaceholder')"
              value-key="uuid"
              label-key="name"
              filterable
              clearable
              required
            />
          </div>
          <!-- Department -->
          <div class="col-span-12">
            <InputReadonly
              v-if="isReadOnly"
              id="department"
              name="department"
              :label="t('settings.municipality.form.department')"
              :placeholder="t('settings.municipality.form.departmentPlaceholder')"
              :modelValue="form.department"
              readonly
            />
            <SingleSelect
              v-else
              id="department"
              name="department"
              v-model="form.department"
              :options="departments"
              :label="t('settings.municipality.form.department')"
              :placeholder="t('settings.municipality.form.departmentPlaceholder')"
              :error="form.errors.get('department')"
              :control-class="'px-3 py-2.5'"
              :dropdown-class="'max-h-60'"
              :option-class="'text-sm'"
              :empty-message="t('common.select.noResults')"
              :search-placeholder="t('common.select.searchPlaceholder')"
              value-key="uuid"
              label-key="name"
              filterable
              clearable
              required
            />
          </div>

          <!-- Name -->
          <div class="col-span-12">
            <InputReadonly
              v-if="isReadOnly"
              id="name"
              name="name"
              :label="t('settings.municipality.form.name')"
              :placeholder="t('settings.municipality.form.namePlaceholder')"
              :modelValue="form.name"
              readonly
            />
            <Input
              v-else
              id="name"
              name="name"
              v-model="form.name"
              :label="t('settings.municipality.form.name')"
              :placeholder="t('settings.municipality.form.namePlaceholder')"
              :error="form.errors.get('name')"
              :form="form"
              required
            />
          </div>

          <!-- Latitude -->
          <div class="col-span-12">
            <InputReadonly
              v-if="isReadOnly"
              id="latitude"
              name="latitude"
              :label="t('settings.region.form.latitude')"
              :placeholder="t('settings.region.form.latitudePlaceholder')"
              :modelValue="form.latitude"
              readonly
            />
            <Input
              v-else
              id="latitude"
              name="latitude"
              v-model="form.latitude"
              :label="t('settings.region.form.latitude')"
              :placeholder="t('settings.region.form.latitudePlaceholder')"
              :error="form.errors.get('latitude')"
              :form="form"
            />
          </div>

          <!-- Longitude -->
          <div class="col-span-12">
            <InputReadonly
              v-if="isReadOnly"
              id="longitude"
              name="longitude"
              :label="t('settings.region.form.longitude')"
              :placeholder="t('settings.region.form.longitudePlaceholder')"
              :modelValue="form.longitude"
              readonly
            />
            <Input
              v-else
              id="longitude"
              name="longitude"
              v-model="form.longitude"
              :label="t('settings.region.form.longitude')"
              :placeholder="t('settings.region.form.longitudePlaceholder')"
              :error="form.errors.get('longitude')"
              :form="form"
            />
          </div>

          <!-- Status -->
          <div class="col-span-12 flex items-center">
            <input
              id="status"
              type="checkbox"
              v-model="form.status"
              :disabled="isReadOnly"
              class="checkbox-primary h-5 w-5 mr-2 disabled:checkbox-disabled disabled:!bg-gray-100"
            />
            <label for="status" class="text-gray-700">
              {{ t('common.form.status') }}
            </label>
          </div>
        </div>
      </form>
    </template>

    <template #buttons>
      <div class="flex gap-4 items-center">
        <Button
          v-if="!isReadOnly"
          variant="secondary"
          customClass="min-w-[100px] bg-gray-300 text-gray-700 px-3 rounded-lg hover:bg-gray-400 transition-all flex justify-center"
          @click="closeModal"
        >
          {{ t('common.modal.buttons.cancel') }}
        </Button>

        <Button
          v-if="!isReadOnly"
          variant="primary"
          customClass="min-w-[100px] bg-primary-500 text-white px-3 rounded-lg hover:bg-primary-600 transition-all flex justify-center"
          @click="onSubmit"
          type="submit"
        >
          {{
            context === 'create' ? t('common.modal.buttons.create') : t('common.modal.buttons.save')
          }}
        </Button>

        <Button
          v-else
          variant="secondary"
          customClass="min-w-[100px] bg-gray-300 text-gray-700 px-3 rounded hover:bg-gray-400 transition-all flex justify-center"
          @click="closeModal"
        >
          {{ t('common.modal.buttons.close') }}
        </Button>
      </div>
    </template>
  </CustomModal>
</template>

<script setup>
import { useI18n } from 'vue-i18n';
import { useSettingsMunicipalityStore } from '@/store';
import { ref } from 'vue';
const store = useSettingsMunicipalityStore();
const { t } = useI18n();
const emit = defineEmits(['success']);

const form = store.form;
const isModalOpen = ref(false);
const context = ref('create');
const requirements = ref({ regions: [] });
const departments = ref([]);

const isReadOnly = computed(() => context.value === 'view');
const modalTitle = computed(() => {
  switch (context.value) {
    case 'create':
      return t('settings.municipality.modalTitleCreation');
    case 'edit':
      return t('settings.municipality.modalTitleModification');
    case 'view':
    default:
      return t('settings.municipality.modalTitleOverview');
  }
});

const closeModal = () => {
  isModalOpen.value = false;
};

const onSubmit = async () => {
  closeModal();
  try {
    const result = context.value === 'create' ? await store.create() : await store.update(form.id);
    onSuccess(result);
  } catch (error) {
    onError(error);
  }
};

const onSuccess = (result) => {
  store.resetForm();
  closeModal();
  emit('success', result);
};

const onError = (error) => {
  console.error('Error submitting form:', error);
  isModalOpen.value = true;
};

const loadRequirements = async () => {
  try {
    const result = await store.requirements();
    requirements.value = result;
  } catch (err) {
    console.error('Error loading requirements:', err);
  }
};

const openCreateModal = async () => {
  context.value = 'create';
  store.resetForm();
  await loadRequirements();
  isModalOpen.value = true;
};

const openEditModal = async (id) => {
  context.value = 'edit';
  store.resetForm();

  try {
    await Promise.all([loadRequirements(), store.find(id, context.value)]);
    if (form.region) {
      onChangeRegion(form.region);
    }
    isModalOpen.value = true;
  } catch (err) {
    console.error('Failed to open edit modal:', err);
  }
};

const openViewModal = async (id) => {
  context.value = 'view';
  store.resetForm();

  try {
    await store.find(id, context.value);
    if (form.region) {
      onChangeRegion(form.region);
    }
    isModalOpen.value = true;
  } catch (err) {
    console.error('Failed to open view modal:', err);
  }
};

const onChangeRegion = (regionUuid) => {
  departments.value = [];
  if (!regionUuid) {
    return;
  }
  const deps = store.regions.find((item) => item.uuid === regionUuid);
  if (deps) {
    departments.value = deps.departments;
  }
};

// onMounted(() => {
//   if (form.region) {
//     onChangeRegion(form.region);
//   }
// });
defineExpose({ openCreateModal, openEditModal, openViewModal });
</script>
