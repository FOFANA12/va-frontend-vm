...
export function getColumns({ t, onEdit, onDelete, onView, formatCurrency, formatNumber }) {
  return [
    ...
    columnHelper.accessor('reference', {
      header: () => t('activity.table.reference'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('name', {
      header: () => t('activity.table.name'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('start_date', {
      header: () => t('activity.table.startDate'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('end_date', {
      header: () => t('activity.table.endDate'),
      cell: (info) => info.getValue(),
    }),

    columnHelper.accessor('responsible', {
      header: () => t('activity.table.responsible'),
      cell: (info) => info.getValue(),
    }),
