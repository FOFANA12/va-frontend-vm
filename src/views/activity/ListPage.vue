<template>
  <DefaultLayout>
    ...
    >
      <!-- Breadcrumbs -->
      <Breadcrumbs :breadcrumbs="breadcrumbs" :pageTitle="t('activity.list.breadTitle')" />

      <!-- Action buttons -->
      <div class="flex justify-end mt-4 mb-4 gap-4">
        <Button
          v-if="hasPermission(PERMISSIONS.DELETE_ACTIVITY) && selectedRows.length > 0"
          :icon="Trash"
        ...
        <LinkButton
          v-if="hasPermission(PERMISSIONS.CREATE_ACTIVITY)"
          :to="{ name: 'activity-create' }"
          :icon="Plus"
          variant="primary"
          customClass="sm:px-4"
        >
...
const breadcrumbs = [
  ...
  {
    title: 'activity.list.breadActive',
    disabled: true,
    to: {
      name: 'activity',
    },
...
const columns = getColumns({
  t,
  onView: (id) => router.push({ name: 'activity-show', params: { id } }),
  onEdit: (id) => router.push({ name: 'activity-edit', params: { id } }),
